...
component displayname="Geocoder.ca CFML"  {
  ...
  variables._geocodercacfml_version = '0.0.0';
  ...
  /**
  * @hint Catch-all wrapper for manually using all api parameters. See the docs for what the parameters can be.
  */
  public struct function geocode( struct params = {} ) {
    var queryParams = params;
    if( params.isEmpty() ) {
      queryParams = arguments.copy();
      queryParams.delete( 'params' );
    }
    return apiCall( 'GET', '/', queryParams );
  }

  /**
  * @hint Convenience method for using the locate param to search.
  * @input "The location may be a street address, an intersection, a zip code, an ip address or any other combination of location entities. for example 330 metcalfe ottawa or metcalfe & wellington ottawa"
  */
  public struct function locate( required string input, struct params = {} ) {
    var queryParams = params;
    if( params.isEmpty() ) {
      queryParams = arguments.copy();
      queryParams.delete( 'params' );
    ...
    queryParams[ 'locate' ] = input;
    return apiCall( 'GET', '/', queryParams );
  }

  /**
  * @hint Convenience method for using the scantext param to search.
  * @input "Free form text containing locations. for example: This and That and the Other street in Porters Lake Nova Scotia"
  */
  public struct function scantext( required string input, struct params = {} ) {
    var queryParams = params;
    if( params.isEmpty() ) {
      queryParams = arguments.copy();
      queryParams.delete( 'params' );
  ...
  private struct function apiCall(
    ...
    if( len( variables.authCode ) ) queryParams[ 'auth' ] = variables.authCode;
    //hardcoded for JSON
    queryParams[ 'json' ] = 1;
  ...
  private struct function getBaseHttpHeaders() {
    return {
      ...
      'User-Agent' : 'Geocoder.ca CFML/#variables._geocodercacfml_version# (ColdFusion)'
    };
  }
